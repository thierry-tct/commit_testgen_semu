...
def main():
    ...
    parser.add_argument("--max_time", type=int, default=4*3600, help="Max test generation time")
    ...
    if is_testgen_only:
        ...
        plotTrend(tech2time2fd, linesplotfile, xlabel="ellapsed time (s)", ylabel=ylabel, yticks_range=yticks_range, \
                                                                            order=sorted(list(tech2time2fd), key=string_num_sort_key), x_as_xticks=True)
        ...
        for metric, data, tech2id2tests in [("FR", id2bugtests, fr_tech2id2tests), ("Differences", id2difftests, diff_tech2id2tests)]:
            ...
            for tech in tech2id2tests:
                for b_id in tech2id2tests[tech]:
                    heatmap_df.append({"Technique": tech, "Commit/Bug ID": b_id, "#Tests": len(tech2id2tests[tech][b_id])})
            heatmap_df = pd.DataFrame(heatmap_df)
            plotHeatmap(heatmap_df, "Commit/Bug ID", "Technique", "#Tests", heatmap_file)
    else:
        # load data
        id2rMSobj = {}
        for d in os.listdir(input_topdir):
